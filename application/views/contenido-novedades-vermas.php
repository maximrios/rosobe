...
if(count($aNovedades)>0): ?>
  ...
  <article class="noticias-home">
    <p class="texto-descripciones"><?=GetDateFromISO($itemNovedad['inicioNovedadRol'])?></p>
    <h3><a href="inicio/novedades/<?=$itemNovedad['idNovedadRol']?>"><?=$itemNovedad['tituloNovedadRol']?></a></h3>
    <p class="texto-descripciones"><?=$itemNovedad['descripcionNovedadRol']?><br />&nbsp;</p>
    <a class="button ok" href="inicio/novedades/<?=$itemNovedad['idNovedadRol']?>">Ver novedad completa</a>
  </article>
  <?php endforeach; ?>
<?php else: ?>
  <article class="noticias-home">
