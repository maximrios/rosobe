<div class="col-lg-9 container">
	<div class="col-lg-12 breadcrumb">
		<label><?=$breadcrumb?></label>
		<!--<h5>Nosotros ::</h5>-->
	</div>
</div>
<div class="col-lg-9 container content">
	<div class="col-lg-12">
		<h3 class="titulo-section">Nuestra empresa</h3>
		<hr class="titulo-section">
	</div>
	<div class="col-lg-6">
		<div id="slider-empresa" class="nivoSlider">
			<img src="assets/images/empresa/imagen1.jpg">
		</div>
	</div>
	<div class="col-lg-6">
		<h5>Quienes Somos</h5>
		<p>Industrias Ro.So.Be. S.R.L. nació como resultado de la inquitud de sus titulares, de ofrecer un nuevo servicio en el mercado de fabricación de muebles y aberturas.
Pasa a paso, con mucho esfuerzo y trabajo, en base a la experiencia obtenida en cada trabajo realizado y el estudio concienzudo de las nuevas tecnologías, llegamos a mecanizar el proceso de fabricación con la mas moderna y sofisticadas maquinas, siendo un orgullo para los miembros contar con maquinarias computarizadas.-
Contamos además con un Show Room, donde se puede encontrar mercaderías standart y muestras de todos nuestros productos y trabajos, siendo ademas atendidos por gente especializada para desarrollar lo que sus clientes necesitan. Una preocupación importante de la empresa es la variedad de materia prima. Por ello trabajamos con maderas blandas (Cedro, Paraiso, etc.), semiduras (afata, algarrobos, palo blanco, etc.), duras (quina, quebracho, lapacho, etc.), melaminas, aglomerados, aluminio, hierro, etc.</p>
	</div>
	<div class="col-lg-12">
		<h5>Nuestros Objetivos</h5>
 		<p>RoSoBe tiene planteado objetivos claros destinados al servicio que brinda a sus clientes
 		Brindar a nuestros clientes un amplio surtido en muebles y aberturas exclusivas.-
 		Cumplir en tiempo y forma los trabajos encomendados por los clientes.-
 		Lograr la satisfacción plena de la confienza depositada de nuestros clientes.-
 		Crecer y ser mejores día a día sin olvidar de donde vinimos y hacia donde queremos llegar.-</p>
 		<!--<ol>
 			<li>1- Brindar a nuestros clientes un amplio surtido en muebles y aberturas exclusivas.-</li>
 			<li>2- Cumplir en tiempo y forma los trabajos encomendados por los clientes.-</li>
 			<li>3- Lograr la satisfacción plena de la confienza depositada de nuestros clientes.-</li>
 			<li>4- Contar con la capacitación y especialización necesaria para que el cliente encuentre en RoSoBe respuestas a sus necesidades.-</li>
 			<li>5- Crecer y ser mejores día a día sin olvidar de donde vinimos y hacia donde queremos llegar.-</li>
 		</ol>	-->
	</div>
	<div class="col-lg-12">
		<h3 class="titulo-section">Empresas que confian en nosotros</h3>
		<hr class="titulo-section">
		<ul>
			<li></li>
		</ul>
	</div>
</div>
<script type="text/javascript">
$('#slider-empresa').nivoSlider({
	directionNav: false,
	controlNav: false,
});
</script>