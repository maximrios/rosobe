...
class Agentes extends Ext_crud_Controller {
	...
	public function listado($idAgente = 0) {
        ...
        $usuario = '<a href="administrator/usuarios/formulario/{idAgente}" title="Editar usuario de {nombrePersona}" class="icono-usuario usuario" rel="{\'idAgente\': {idAgente}}">&nbsp;</a>';
        ...
        if($this->lib_autenticacion->idRol() == 1) {
        	$tamano = 128;
        	$acciones = $usuario.$ver.$editar.$mensaje;
        }
...
*/
    ...
    public function reporte($estructura=1, $filtro='') {
        $aData['agentes'] = $this->agente->obtener($filtro, $estructura);
        $this->config->set_item('page_orientation', 'P');
        if($estructura == 1) {
            $this->load->view('administrator/sigep/reportes/repo-agentes', $aData);
            //redirect('administrator/agentes/error');
        }
